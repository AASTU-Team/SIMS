...
export const ActivateRegistrationStatus = async (
  req: Request,
  res: Response
) => {
...
export const DeactivateRegistrationStatus = async (
  req: Request,
  res: Response
) => {
  const id = req.body.id;
  ...
  try {
    ...
    if (RegStatus) {
      ...
      return res
        .status(400)
        .json({ message: "unable to find registration status" });
    }
  } catch (error: any) {
    console.log(error.message);
    return res.status(500).json({ message: error.message });
...
export const updateStatus = async (req: Request, res: Response) => {
  try {
    ...
    const updates = await RegistrationStatus.findByIdAndUpdate(
      id,
      requestData,
      {
        new: true,
