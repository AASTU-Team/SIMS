...
import {
  confirmAllRegistrationDepartment,
  confirmRegistrationDepartment,
  getRegistrationDepartment,
  rejectRegistrationDepartment,
...
export default function Registration() {
  ...
  const query = useQuery({
    queryKey: ["studentRegisteredDepartment"],
    queryFn: () => getRegistrationDepartment(user.department),
  });
  console.log(query);

  const ApproveRequestMutation = useMutation({
    mutationKey: ["approveRequestDepartment"],
    mutationFn: (id: string) => confirmRegistrationDepartment(id),
    onError: () => {
  ...
  const ApproveAllRequestMutation = useMutation({
    mutationKey: ["approveAllRequestDepartment"],
    mutationFn: () => confirmAllRegistrationDepartment(user.department),
    onError: () => {
      notification.error({ message: "Registration Not Successful" });
    },
    onSuccess: () => {
  ...
  const RejectRequestMutation = useMutation({
    ...
    mutationFn: ({ id, reason }: { id: string; reason: string }) =>
      rejectRegistrationDepartment(id, reason,user.department),
    onError: () => {
  ...
  return (
    <div className="pt-2">
      <div className="flex justify-end">
        ...
        >
          <button
            className="flex justify-center items-center gap-2 rounded-lg bg-primary px-4 py-2 font-medium text-gray mb-2 hover:bg-opacity-90 "
          >
