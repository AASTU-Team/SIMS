...
import { getDepartmentRequests } from "../../api/grade";
import { GradeApproval } from "../../type/registration";
import Loader from "../../components/Loader";



export default function DepartmentGradeApproval() {


  const query = useQuery({
      queryKey: ["getDepartmentGradeRequest"],
  ...
  const data:GradeApproval[] = []
   ...
   if (
     query.isSuccess &&
     query.data?.data?.message && Array.isArray(query.data?.data?.message) && query.data?.data?.message?.length > 0
   ) {
    
     for (let i = 0; i < query.data?.data?.message?.length; i++) {
       const instructor =query.data?.data?.message[i]?.grade_id?.instructor_id?.name
       const course =query.data?.data?.message[i]?.grade_id?.course_id?.name
      const prev = data?.find((value: GradeApproval) => value.instructor_name === instructor && value.course_name === course);
